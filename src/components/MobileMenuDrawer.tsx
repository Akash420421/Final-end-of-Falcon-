...
export const MobileMenuDrawer: React.FC<MobileMenuDrawerProps> = ({
  ...
  // Robust Body Scroll Lock: completely prevents background scroll/rubber-band on mobile swipe
  useEffect(() => {
    if (isOpen) {
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <div
          className="fixed inset-0 z-50 flex pointer-events-auto touch-none select-none"
          aria-modal="true"
          role="dialog"
          onTouchMove={(e) => {
            // Prevent background touch gestures like pull-to-refresh
        ...
        >
          ...
          <motion.div
            ...
            className="relative w-[85%] max-w-xs bg-[#101124] text-white h-[100dvh] shadow-2xl flex flex-col justify-between z-10 border-r border-white/10 overflow-hidden"
            onTouchMove={(e) => e.stopPropagation()}
          >
            {/* Scrollable Drawer Content */}
            <div className="flex-1 overflow-y-auto overscroll-contain p-5 no-scrollbar flex flex-col justify-between space-y-6">
              <div>
                {/* Header */}
